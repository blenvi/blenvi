...
async function fetchBranchesPage(
  ...
  const url = `${EXTERNAL_API_BASE_URLS.neonConsole}${NEON_API_ENDPOINTS.branches(projectId)}?${params}`;
...
async function fetchOrganizationId(
  ...
  const url = `${EXTERNAL_API_BASE_URLS.neonConsole}${NEON_API_ENDPOINTS.project(projectId)}`;
  const res = await fetch(url, {
    headers: {
      Authorization: `Bearer ${apiKey}`,
      Accept: "application/json",
...
async function fetchComputeUnitSeconds(
  ...
  const to = new Date();
  const from = new Date(
    to.getTime() - POLLING_LIMITS.daysPerMonth * TIME_MS.day,
  );
  ...
  const url = `${EXTERNAL_API_BASE_URLS.neonConsole}${NEON_API_ENDPOINTS.consumptionHistoryV2Projects(params.toString())}`;
  const res = await fetch(url, {
    headers: {
      Authorization: `Bearer ${apiKey}`,
      Accept: "application/json",
  ...
  if (!projectId || !apiKey) {
    return {
      ...
      raw_payload: {
        ...
        detail: UI_TEXT.neon.credentialsRequired,
      },
      response_code: null,
      error_message: UI_TEXT.neon.credentialsRequired,
    };
  }
