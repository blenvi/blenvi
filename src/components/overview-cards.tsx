...
export default function OverviewCards() {
  return (
    <div className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-2 @6xl/main:grid-cols-4">
      {data.map(item => (
        <Card className="@container/card" key={item.id}>
          ...
          <CardFooter className="h-full flex-col items-start justify-between gap-4 text-sm">
