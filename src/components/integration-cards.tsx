...
export function IntegrationCards() {
  ...
  const categories = [
    "all",
    ...new Set(integrations.map((i) => i.category.toLowerCase())),
  ];

  const filteredAndSortedIntegrations = sortIntegrations(
    filter === "all" ? integrations : (
      integrations.filter((i) => i.category.toLowerCase() === filter)
    ),
    sortBy
  );
          ...
          <Card key={integration.id} className="relative overflow-hidden">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <div
                    className={`p-2 rounded-lg border shadow-inner bg-secondary`}
                  >
